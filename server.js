...
名字:5~12字，有世界观辨识度
背景:30~60字，必须包含具体地名。**禁止使用任何具体数字**（不要写人数、杀敌数、战役天数、编制数等）；用氛围、光线、声音、当地传说、身份描述来塑造兵团的风格与气场
信念:10~20字，兵团的核心精神
...
- **背景里禁止写具体数字来描述人数或战果。** 不要写"三百""五百""上千"等整数，也不要写"六十七""四十七骑"等不规则数字；直接用氛围、光线、声音、传说来塑造画面感，不写人数反而更有神秘感。

【背景写作参考——五种叙事风格，学习其手法而非内容】
下面展示五种不同的叙事笔法。**你要学的是它们的写作手法（结构、视角、节奏），但必须自己创作全新的故事——不同的世界观、不同的地名、不同的人物，绝不重复示例中的任何细节。背景中禁止使用具体数字（人数、天数、杀敌数、编制数等），用氛围和传说塑造神秘感。**

风格A——**史诗叙事，画面感**：开门见山定调，用动作细节构建画面感，结尾用留白制造余韵。
> 黑岩堡的矮人从不投降。当兽人大军撞开城门的那一刻，铁卫们以血肉为盾死守不退，直到晨曦中传来氏族援军的号角，城门后的幸存者浑身是血、沉默地举着残破的旗帜。

风格B——**名声梗，从他者视角**：用"人人都知道"的口吻引入，侧面烘托兵团的威名，再用一个具体战绩证明。
> 谁都知道"不破防线"这个名号。帕兰诺平原上，这支步兵硬扛无数波冲击而阵线纹丝未动。战后清点，将士死了大半，没有一个活人后退过一步。

风格C——**氛围侧写，禁"曾经/曾在"**：用环境、声音、光线来暗示兵团的风格，不直接说他们强，而是让读者自己感受到。完全不需要数字。
...
风格E——**传说隐喻，神秘感**：借用当地传说、迷信或自然异象来暗示兵团的可怕，全程不写具体人数和战绩。
> 猩红沼泽终年不散的雾气是他们的斗篷。当敌军听见泥泞中传来马蹄声时，往往已经来不及调整阵型。当地人相信，这支骑兵根本不是活人，而是沼泽深处某种古老意志的化身。

【输出格式示例——只展示排版结构，不要求内容雷同】
类型:步兵
名字:（自定义）
背景:（自定义，用上面五种风格之一创作全新的故事，禁止使用具体数字）
信念:（自定义）
主武器名字:（自定义）
主武器类型:（自定义）
护甲名字:（自定义）
...
2. **叙事驱动身份**：在这个时空背景下，这支兵团是谁？他们的来历、身份、风格是什么样的？让背景故事产生一种"这支部队真实存在过"的厚重感。**注意：背景里不需要写具体数字**——不写杀敌数、不写人数、不写战役持续天数，用氛围、声音、光线、材质、地方传说来塑造兵团的神秘感。
3. **装备的艺术化衍生**：装备的名字和配置不应是随机的，而应是背景故事的延伸。如果是贫民军，装备可能带有"生锈""拼凑"的词缀；如果是神选者，装备应体现其超凡脱俗的工艺。

【装备命名建议——追求辨识度与沉浸感】
- 避免廉价的网文风命名（如：神级战斧、至尊盔甲）。
- 尝试通过地名、材质、风格特征或当地传说来命名，避免直接用"战绩"作为前缀。
- 示例：从"铁剑"进化为"白城禁卫钢剑"；从"皮甲"进化为"林影游侠轻铠"。

【数值分配的艺术】
你给出的每一项装备数字（武器伤害/破甲/护甲防御）都应根据背景风格来设定：
- 如果你描述该兵团"势不可挡"，其伤害应偏向区间上限。
- 如果描述其"甲胄厚重但行动迟缓"，则高防御与低移动速度相匹配。
- 数字的多少、整齐与否与背景故事完全无关，背景里**不要使用具体数字**。
- 请在给定的数值区间内，根据你创作的逻辑自由微调，让数值成为故事的一部分。

【创作灵感参考（不限于此）】
你可以尝试：史诗叙事（宏大的战场侧写）、他者视角（侧面烘托其恐怖或伟大）、氛围侧写（通过细节描写如月光、箭雨、呼吸声来传达风格）、反差叙事（看似弱小实则坚韧），或传说隐喻（用地方传说和迷信来暗示兵团的可怕）。**所有风格都应当避免在背景中使用具体数字**。
...
现在开始生成，只输出key:value格式，不输出其他内容。`;
  ...
  try {
    ...
    // ===== 服务端兜底：背景和信念为空时自动生成（全部使用无数字风格）=====
    const BG_POOL = [
      // 风格A：史诗画面
      '安其拉之门轰然碎裂的那一刻，远征军先锋连没有犹豫。他们迎着潮水般的虫群冲锋，用血肉在城门残骸上筑起第二道墙——直到援军赶到，墙还立着',
      '黑岩堡的矮人从不投降。当铁炉堡援军踏碎晨曦赶到时，城门后的幸存者浑身是血、沉默地举着残破的氏族旗帜',
      // 风格B：名声与他者视角
      '谁都知道"不破防线"这个名号。帕兰诺平原上，这支步兵硬扛了无数次冲击而阵线纹丝未动。战后清点，没有一个活人后退过一步',
      '洛汗草原上的牧民至今还在传唱骠骑精锐的故事：号角堡陷落前最后一刻，骠骑从侧翼杀入兽人大军。歌里说他们的战马都染成了红色',
      '北境边民有个说法——宁可遇上一整支混沌军团，也不要撞见黑色守望者的巡逻队。没人能说清他们的来历，但他们的旗帜所到之处，冰原上的尸骨都会被肃清',
      // 风格C：氛围侧写
      '月下掠过的白影，林间破空的轻响——灰谷长弓兵团不爱在白天打仗。月光比阳光更不会出卖弓弦的颤动，而精灵的箭头从不失手',
      '没人见过他们整队出发的样子。银月城哨兵永远只出现在你需要他们的地方——通常在敌人发现之前就已经射完了首轮箭雨',
      '猩红沼泽终年不散的雾气是他们的斗篷。当敌军听见泥泞中传来马蹄声时，往往已经来不及调整阵型',
      // 风格D：反差叙述
      '按兵部账本，十字军的编制早该撤销了——成员不是残兵就是囚犯，装备全是淘汰货。但奇怪的是，每次王国有难，这支破烂兵团总能活着回来。而且打赢',
      '吸血鬼伯爵从不承认希尔瓦尼亚亡者军团属于他的统治——他们不服从任何贵族，也不效忠任何王权。但每逢亡灵天灾入侵时，最先挡在平民前方的永远是这支无名亡者',
      '恐虐的祭司看不起这支连仪式铠甲都没有的散兵。然而混沌荒原上流传着一句诅咒：每个见过他们的敌人都再也没有回来过',
      // 风格E：纯氛围/传说
      '他们的铠甲上刻满了看不懂的符文，有人说那是失传的古语。每逢血月之夜，符文会发出幽蓝的微光，而敌人的刀刃总是在那时变钝',
      '白城禁卫从不在公众场合摘下头盔。坊间流传着一种说法：头盔下面没有面孔，只有永不熄灭的誓言',
      '锈水湾的海盗们宁可自相残杀也不愿招惹这支舰队。不是因为他们船坚炮利，而是他们的船长据说能和风暴对话',
      '铁脊山脉深处有一支不为人知的守军。当地猎人发誓，他们从没见过这支部队下山，但每当山口出现异响，次日雪原上总会多出几具冻僵的兽人尸体'
    ];
    const BELIEF_POOL = [
...
类型: [步兵/骑兵/远程兵/空军]
名字: 4~10字，有世界观辨识度
背景: 30~60字，必须包含该世界观中的具体地名。**禁止使用任何具体数字**（不要写人数、杀敌数、战役天数、编制数等）；用氛围、声音、当地传说、身份描述来塑造兵团的风格与气场
信念: 10~20字，兵团的核心精神
